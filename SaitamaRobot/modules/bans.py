...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Trying to put me against Papa? I should ban you!")
            return log_message
            ...
            message.reply_text(
                "I better not, it's dangerous to do that here.")
            return log_message
...
def temp_ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("I don't wanna, baka.")
        return log_message
...
def boop(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("I really wish I could....")
        return log_message
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Booped Out! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
        ...
        message.reply_text("Well damn, I just can't do it.")
...
def boopme(update: Update, context: CallbackContext):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text(
            "You're an admin so I can't do that, yet.")
        return

    res = update.effective_chat.unban_member(
        user_id)  # unban on current user = kick
    if res:
        update.effective_message.reply_text("*Thanos snaps you out of existence*")
    else:
...
__help__ = """
 • `/boopme`*:* boops the user who issued the command
...
PUNCHME_HANDLER = DisableAbleCommandHandler(
    "boopme", punchme, filters=Filters.group)
